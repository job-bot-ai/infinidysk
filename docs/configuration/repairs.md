...
| Health Check Depth | `repair.healthcheck-depth` | `standard` | quick / standard / enhanced / deep / complete. `quick` STATs only ~two dozen head/tail/stride segments per file regardless of size (bypassing the 8000-segment floor and the size/age curve) — a fast first pass that reliably tells an intact posting from one lost to takedown or retention. Because it never covers a whole file, `quick` checks do not run damage classification (see below); pair it with a periodic deeper pass. |
